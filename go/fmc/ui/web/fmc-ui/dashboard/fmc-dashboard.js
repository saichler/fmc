...
(function() {
    ...
    function _renderKpiRow(container, kpis) {
        var pending = kpis.length;
        var results = new Array(kpis.length);
        ...
        kpis.forEach(function(kpi, idx) {
            ...
            Layer8DPortalDashboard.fetchCount(kpi.endpoint, q, function(total) {
                ...
                results[idx] = Layer8DWidget.render(
                    { label: kpi.label, iconSvg: kpi.iconSvg, onClick: "FmcDashboard._nav('" + section + "')" },
                    total,
                    {}
                );
                pending--;
    ...
    function _renderActions(container) {
        if (!container) return;
        var actions = [
            { label: 'Add Member', section: 'core' },
            { label: 'Schedule Session', section: 'coaching' },
            { label: 'View Weight Logs', section: 'progress' }
        ];
        var html = '<div style="display:flex;gap:12px;flex-wrap:wrap;">';
        actions.forEach(function(a) {
            html += '<button class="layer8d-btn layer8d-btn-secondary layer8d-btn-small" ' +
                    'onclick="FmcDashboard._nav(\'' + a.section + '\')">' +
                    a.label + '</button>';
        });
        html += '</div>';
    ...
    function initializeFmcDashboard() {
        ...
        var actionsContainer = document.getElementById('fmc-dashboard-actions');

        if (kpiContainer) _renderKpiRow(kpiContainer, PRIMARY_KPIS);
        if (statsContainer) _renderKpiRow(statsContainer, SECONDARY_KPIS);
        _renderActions(actionsContainer);
    }

    window.initializeFmcDashboard = initializeFmcDashboard;
    window.FmcDashboard = { _nav: _navigateToSection };
})();
